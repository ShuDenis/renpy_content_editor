import type { SceneProject, Hotspot } from './sceneSchema';

export function round3(value: number): number {
  return Math.round(value * 1000) / 1000;
}

export function projectToCanvasScalar(
  proj: SceneProject,
  value: number,
  canvasSize: number,
  refSize: number,
): number {
  return proj.project.coords_mode === 'relative'
    ? value * canvasSize
    : value * (canvasSize / refSize);
}

export function canvasToProjectScalar(
  proj: SceneProject,
  value: number,
  canvasSize: number,
  refSize: number,
  ...
  const v =
    proj.project.coords_mode === 'relative'
      ? value / canvasSize
      : value / (canvasSize / refSize);
  return round3(v);
}

export function projectToCanvasPoint(
  proj: SceneProject,
  x: number,
  y: number,
  canvasW: number,
...
export function canvasToProjectPoint(
  proj: SceneProject,
  x: number,
  y: number,
  canvasW: number,
...
export function convertProjectCoordsMode(
  proj: SceneProject,
