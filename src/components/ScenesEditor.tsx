...
export default function ScenesEditor() {
  const [proj, setProj, resetProj, undo, redo] = useUndoState<SceneProject>(
    emptyProject(),
    validateSceneProject,
  );
  ...
  useEffect(() => {
    fetchScenes()
      ...
      .catch((e: any) => {
        setStatus("Не удалось загрузить сцены: " + e.message);
      });
  }, []);
  ...
  function onSaveClicked() {
    ...
    if (!scene) return;
    saveScene(scene)
      .then(() => setStatus("Сцена сохранена"))
      .catch((e: any) => setStatus("Ошибка сохранения: " + e.message));
  }
